...
### Step 3: 検証（Evaluator）
- GeekPowerCode のプロジェクトルールに従って、実装されたスプリントを評価する
...
6. **確認なしで自動パイプラインを実行する** - Generator 完了後は自動的に Evaluator を起動する。Evaluator 合格後は自動的に次のスプリントの Generator を起動する。「〜しますか？」というユーザーへの確認は不要。ただし以下の場合のみユーザーに確認する：全スプリントが完了したとき／仕様書が存在しないとき／ユーザーが明示的に「止めて」「確認して」と指示したとき。
